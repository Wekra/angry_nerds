...
class SingleCollectionOperationStreamBuilder<T extends Identifiable> with StreamSubscriberMixin<Event> {
  final List<T> _items = new List();
  final StreamController<ListOperation<T>> _controller = new StreamController();
  bool _connectedEventDispatched = false;
  ...
  SingleCollectionOperationStreamBuilder(this.itemQuery, this.itemMapper) {
    _controller.onCancel = cancelSubscriptions;
    _controller.onListen = _listenToFirebaseEvents;
  }

  void _listenToFirebaseEvents() {
    listen(itemQuery.onValue, _onValue, onError: _onError);
    listen(itemQuery.onChildAdded, _onItemAdded, onError: _onError);
    listen(itemQuery.onChildRemoved, _onItemRemoved, onError: _onError);
    listen(itemQuery.onChildMoved, _onItemMoved, onError: _onError);
    listen(itemQuery.onChildChanged, _onItemMoved, onError: _onError);
  }
